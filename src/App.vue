...
<script>
	export default {
		...
		computed: {
			percentHydration() { return this.hydration / 100 },
			flour() { return Math.round(this.tdw / (1 + this.percentHydration + 0.025))},
			water() { return Math.round(this.flour * this.percentHydration) },
			salt() { return Math.round(this.flour * 0.025)},
			starter() { return Math.round(this.flour * 0.2)},
			outputs() {
				return {
